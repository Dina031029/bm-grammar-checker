<script>
    ...
    // Merge logic: ensure we use the correct DB names
    $: notes = data?.notes || [];
...
<div class="note-container">
    ...
    {#if selectedPdf}
        <div class="pdf-viewer-overlay" on:click={closePreview}>
            <div class="pdf-content" on:click|stopPropagation>
                ...
                <div class="pdf-footer">
                    <p>Masalah memaparkan? <a href={selectedPdf} target="_blank">Buka di Tab Baru</a></p>
                </div>
            </div>
        </div>
    {/if}
        ...
        {#each filteredNotes as note}
            <div class="note-card">
                ...
                <div class="note-body">
                    ...
                    <p class="date">Dimuat naik: {new Date(note.created_at).toLocaleDateString()}</p>
                    
                    <div class="actions">
                        <button class="view-btn" on:click={() => viewPdf(note.file_url)}>
                            Lihat Nota
                        </button>
...
<style>
    ...
    .search-bar input { 
        ...
        border-radius: 15px; font-size: 1rem; box-sizing: border-box;
    ...
    .image-box { 
        width: 100%; 
        height: 200px; /* Fixed height for the cover area */
        background: #f1f5f9;
        overflow: hidden;
    }
    .cover-img { 
    ...
    .category-tag { 
        ...
        background: #3b82f6; padding: 4px 12px; border-radius: 50px;
        text-transform: uppercase; margin-bottom: 12px;
    }

    .note-title { 
        font-size: 1.3rem; color: #1e293b; line-height: 1.4; 
        margin: 0 0 8px 0; font-weight: 700;
        /* Title is NOT truncated - it will wrap to new lines */
    }
    ...
    /* Actions */
    .actions { 
        margin-top: auto; /* Pushes buttons to bottom of body */
